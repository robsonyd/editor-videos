...
rm -rf "$BUILD_DIR"
mkdir -p "$MACOS_DIR" "$RESOURCES_DIR" "$SOURCE_DIR" "$SCRIPTS_DIR" "$DIST_DIR"
...
bootstrap_python() {
  ...
  if [ ! -x "$PYTHON_BIN" ]; then
    ...
    if [ -z "$system_python" ]; then
      show_error "Não encontrei Python 3.10+ neste Mac. Instale Python 3.10+ ou peça ao time de TI para preparar o Mac antes de abrir o EVR Deluxe."
      exit 1
    fi
